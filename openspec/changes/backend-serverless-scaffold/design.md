...
**再次調整（依領域縱切，取代原先「先分層、層裡面再分領域」的 `StepGo.Domain`/`StepGo.Application`/`StepGo.Infrastructure` 三個橫向專案）**：原始分層讓每個專案內部塞了 8 個 capability 的資料夾，領域邊界只活在檔案路徑習慣裡，編譯器不會強制。改為依領域切成 `StepGo.Identity`/`StepGo.Courses`/`StepGo.Orders`/`StepGo.Payouts`/`StepGo.RefundTickets`/`StepGo.Governance`/`StepGo.Notifications` 七個專案，每個專案內部再用 `Domain/`、`Application/`、`Infrastructure/` 資料夾分層（命名空間仍是 `StepGo.<Domain>.Domain` 等，只是現在專案邊界＝領域邊界，而不是層邊界）。核對過實際 `using` 關係後，領域之間的相依方向是：`Governance`（無外部相依）←`Identity`←`Notifications`／`Courses`←`Orders`←`RefundTickets`／`Payouts`，全部單向、無環。三個真正跨領域、被所有領域依賴的東西（`AggregateRoot`/`DomainException`/`Money`/`Role`/`RefundRuleSet` 等 kernel 型別、`IClock`/`IEventPublisher`/`ICurrentUserAccessor`/`IObjectStorage` 等 Application port、`DynamoDbKeys`/`StepGoTableOptions`/`AttributeValueExtensions` 等 Dynamo 底層工具）合併進一個 `StepGo.Shared` 專案，同樣用 `Domain/`、`Application/`、`Infrastructure/` 分。`FeeLedger` 原本掛在 Domain/Application 底下卻沒有自己的 aggregate/repository，本質是純計算工具，因此直接併入 `StepGo.Shared`（`Domain/FeeLedger/`、`Application/FeeLedger/`），不獨立成領域專案。`EventBridgePublisher`／`DomainEventJsonContext` 因為要在同一支程式碼裡認得 Orders 與 RefundTickets 兩個領域的具體 Domain Event 類別才能序列化發布，若放進 `StepGo.Shared` 會讓 Shared 反過來依賴領域、破壞「Shared 在圖的最底層」的前提，因此改放進 `StepGo.Api.Shared`（本來就依賴所有領域的組合層）。`Domain` 層「不得依賴 Infrastructure/AWS SDK」的邊界不再由專案參照鏈強制（原本 `StepGo.Domain.csproj` 完全沒有 ProjectReference，寫錯會直接編譯失敗），改成單純靠 code review 自律——這是明知的取捨，換來專案數量大幅減少（`StepGo.Domain`/`StepGo.Application`/`StepGo.Infrastructure` 三個橫向專案變成 8 個領域專案 + 1 個 `StepGo.Shared`）。
